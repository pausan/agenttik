...
- --api GET '/api/schedules?project_id=PROJECT_ID&window=all': list a project's scheduled jobs. GET /api/schedules/JOB_ID includes run history. PATCH /api/schedules/JOB_ID with {"paused":true} pauses a job; false resumes it. --api POST --body '{}' /api/schedules/JOB_ID/run runs it now; {"enqueue":true} waits for the project's other work.
- --api POST --body '{"project_id":PROJECT_ID,"provider":"PROVIDER","model":"MODEL","title":"Hourly check","prompt":"Work to repeat","every":"interval","interval_minutes":60,"remaining":-1}' /api/schedules: create an hourly job. remaining=-1 repeats indefinitely; a positive number limits its runs. Optional account_id, effort and permission work as for tasks.
