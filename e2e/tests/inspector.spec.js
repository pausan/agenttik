...
test("merge and rebase choose a destination and show recovery", async ({ page }) => {
  ...
  const disclosure = pane.locator("summary", { hasText: "Merge / rebase" });
  const destination = pane.getByRole("button", { name: "Destination branch", exact: true });
  const hints = pane.getByText("If there are conflicts, AI will solve them", { exact: false });
  ...
  const buttonBox = await merge.boundingBox();
  const destinationBox = await destination.boundingBox();
  expect(Math.abs(buttonBox.x + buttonBox.width - destinationBox.x - destinationBox.width)).toBeLessThan(2);
  await disclosure.click();
  await expect(merge).toBeHidden();
