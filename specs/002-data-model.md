...
- **sessions** — `id (uuid), project_id, title, provider, account_id,
  ...
  is generated by us
  and passed to `claude --session-id` on a task's first turn, so our id and the
  provider's match for Claude Code. A later turn that starts a fresh CLI
  conversation lets the CLI name it instead, because ours is taken
  ([003](003-providers.md)). Codex assigns its own, kept in `provider_session_id`. `source`
  is always
