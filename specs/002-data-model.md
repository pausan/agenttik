...
- **projects** — `id, name, path (unique), created_at, position`. A project is a
  name and a folder. Nothing is copied; the folder is used as the agent's working
  directory. `position` is the order the Projects sidebar was dragged into; 0
  means a newly added project, which sorts above a manually ordered list.
