...
Every entry also names the subscription it would run on, where the provider
has more than one ([050](050-subscription-accounts.md)); a favourite is a
model and an effort only, so picking one keeps the task on the account it is
already on.
