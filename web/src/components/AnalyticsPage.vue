...
const metrics = [{ label: "Reported cost", value: "cost_usd" }, { label: "Input tokens", value: "input_tokens" }, { label: "Output tokens", value: "output_tokens" }, { label: "Turns", value: "turns" }];
    ...
    <template #body>
      <div class="mx-auto max-w-6xl space-y-6 pb-8">
        <div class="flex flex-wrap items-end gap-3">
          ...
          <label class="space-y-1 text-sm"><span class="block text-muted">Break down by</span><USelect v-model="grouping" :items="[{ label: 'Provider', value: 'provider' }, { label: 'Subscription / connection', value: 'subscription' }]" aria-label="Analytics grouping" class="w-56" /></label>
        ...
        <template v-else-if="data">
          ...
          <section v-for="group in groups" :key="group.key" class="space-y-3" :aria-label="providerLabel(group.provider)">
            <div class="flex flex-wrap items-baseline justify-between gap-2">
              <h2 class="text-lg font-semibold">{{ providerLabel(group.provider) }}<span v-if="group.account_id !== null" class="font-normal text-muted"> · {{ accountLabel(group.provider, group.account_id) }}</span></h2>
          ...
          <p v-if="groups.length" class="text-xs text-muted">* Share of the selected ranking metric within each {{ grouping === 'provider' ? 'provider' : 'subscription / connection' }}. Projects and tasks are ordered highest first.</p>
        </template>
      </div>
    </template>
  </UModal>
